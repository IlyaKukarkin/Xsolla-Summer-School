window.onload = function () {
  ...
  Vue.component('line-chart', {
    ...
    created () {
      ...
      //Собираем список для рейтинга популярности платежных систем
      let resList = []
      for (let i = 0; i < rawData.length; i++) {        
        if (resList.find(x => x.name === rawData[i].transaction.payment_method.name) === undefined) {
          resList.push({"name": rawData[i].transaction.payment_method.name, "count": 1})
        }
        else
        {
          resList.find(x => x.name === rawData[i].transaction.payment_method.name).count++
        }
        this.paymentsCount++
      }
      ...
      for (let i = 0; i < this.paymentsList.length; i++) {
        this.lab.push(this.paymentsList[i].name)
        this.dat.push((this.paymentsList[i].count / this.paymentsCount * 100).toFixed(1))
        this.col.push(getRandomColor())
      }      
    },
    mounted () {
  ...
  class myData {
    constructor(name, method, date, email, amount, purchase) {
      this.transaction.project.name = name;
      this.transaction.payment_method.name = method;
      this.transaction.transfer_date = date;
      this.user.email = email;
      this.payment_details.payment.amount = amount;
      this.purchase.virtual_items = purchase;
    }
  }

  var XsollaProject = new Vue({
    el: '#XsollaProject',
    data: {
      pageSize:10,
      currentPage:1,
    ...
    computed:{
      filteredList() {
        ...
        this.currentPage = 1
        return this.rows.filter(myData => {
          if (myData.user.email == null)
          {
            return (myData.transaction.project.name.toLowerCase().includes(this.search.toLowerCase()) || myData.transaction.payment_method.name.toLowerCase().includes(this.search.toLowerCase()) || str.toLowerCase().includes(this.search.toLowerCase()))
          }
          else
          {
            return (myData.transaction.project.name.toLowerCase().includes(this.search.toLowerCase()) || myData.transaction.payment_method.name.toLowerCase().includes(this.search.toLowerCase()) || myData.user.email.toLowerCase().includes(this.search.toLowerCase()))
          }
        })
      },
      pagedRows:function() {
